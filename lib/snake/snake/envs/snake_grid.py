...
class SnakeGrid(gym.Env):
    ...
    def dead_coordinate(self, index: Index):
        return np.any(
    ...
    def step(self, action: int):
        ...
        # x steps.
        self.dead = self.dead_coordinate(next_head_index)
    ...
    def _render_as_string(observation, current_direction, dead):
        ...
        for state_value in np.unique(observation):
            SnakeGrid._update_string_board_from_observation(
                observation, string_board, state_value, current_direction
            )
